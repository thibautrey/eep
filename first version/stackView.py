...
def load_master_flat_frame(args):
    master_flat = None
...
def load_frame(imgname, pbar):
    ...
    params = rawpy.Params(gamma=(1,1),
                          ...
                          user_black=0 if calibrate or args['--raw'] else None)
    ...
    try:
        ...
        with rawpy.imread(imgname) as raw:
            if calibrate:
                ...
                dark_removed = np.clip(raw.raw_image - master_dark, 0, None) / master_flat
                np.copyto(raw.raw_image, dark_removed.astype(np.uint16))
            
            if args['--raw']:
                img = raw.raw_image.astype(np.float32) / 65535.0
            else:
                pbar.set_postfix(step=f"demosaicing")
                img = raw.postprocess(params).astype(np.float32) / 65535.0
...
def compute_homography(next_image, base_image, frame_num=0, cache=None):
    ...
    if args['--threshold']:
        ...
        np.clip(next_imageGray, 0, 1, out=next_imageGray)

        thr = np.percentile(base_imageGray, args['--threshold'])
        base_imageGray = (base_imageGray - thr) / (1.0 - thr)
        np.clip(base_imageGray, 0, 1, out=base_imageGray)
    ...
    kp2, des2 = None, None
    if cache is not None and (frame_num <= 2 or is_power_of_two(frame_num)):
    ...
    if args['--debug'] >= 3:
        # Draw top matches
        imMatches = cv2.drawMatches(next_image, kp1, base_image, kp2, matches, None)
...
def main():
    ...
    stabilized_sum = np.zeros((shape[0], shape[1]), dtype=np.float32)
    divider_mask_sum = np.zeros((shape[0], shape[1]), dtype=np.float32)

    for f in pbar:
        pbar.set_description(f"Processing image {f}")
        imgname = infiles[f]
        input_image = load_frame(imgname, pbar)
        ...
        input_image_no_border = input_image
        input_image = add_padding(input_image_no_border, args["--padding"])
        imagenumbers.append(f)

        if f == 0:
            shape_original = input_image.shape

        if stabilized_sum is None:
            stabilized_sum = np.float32(input_image)
        ...
        if args['--align'] and f > 0:
            base = (stabilized_sum / divider_mask_sum)
            base = base.clip(0, 1)
        ...
        stabilized_sum += np.float32(transformed_image)
        divider_mask_sum += transformed_mask

        if args['--debug'] >= 3:
            cv2.imwrite(f'DEBUG_{f}.hdr', np.float32(transformed_image))
            cv2.imwrite(f'DEBUG_average_so_far_{f}.hdr', stabilized_sum / divider_mask_sum)
            cv2.imwrite(f'DEBUG_average_so_far_{f}.png', np.uint16(stabilized_sum / divider_mask_sum * 65535))
            cv2.imwrite(f'DEBUG_divider_mask_{f}.png', np.uint16(divider_mask_sum))
            print('mask: ', divider_mask_sum[0, 0])

    stabilized_average = stabilized_sum
divider_mask = divider_mask_sum
